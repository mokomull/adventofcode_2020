...
fn setup_cups(input: &[usize]) -> Vec<Cup> {
    ...
    cups[input[0]].previous = *input.last().unwrap();
...
fn run(iterations: i32, input: &[usize]) -> Vec<usize> {
    ...
    for _ in 0..iterations {
        ...
        cups[next_after_this_round].previous = current_cup;
        ...
        cups[beginning_of_range].previous = destination;
        cups[end_of_range].next = next_after_destination;
        cups[next_after_destination].previous = end_of_range;
